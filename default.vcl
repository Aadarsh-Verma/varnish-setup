    ...
    if ( var.global_get("page_identifier") != "uncacheable" && resp.http.X-Cache-Status == "HIT" ){
        ...
        if(!cookie.isset("GOOGLE_SEARCH_ID")){
            ...
            if(!req.http.x-visitor_id){
                set resp.http.x-visitor-id = regsub(var.global_get("GOOGLE_SEARCH_ID"),"(?:(?:^|.*;\s*)GOOGLE_SEARCH_ID\s*\=\s*([^;]*).*$)|^.*$", "\1");
...
sub set_cookie {
    ...
    #if(str.count(req.http.Cookie) > 1){
     if(req.http.Cookie != ""){
